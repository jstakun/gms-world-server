...
public class GeocellManager {
   ...
   public static final <T> List<T> proximitySearch(Point center, int maxResults, double maxDistance, Class<T> entityClass, GeocellQuery baseQuery, GeocellQueryEngine queryEngine, int maxGeocellResolution) {
       ...
       while(!curGeocells.isEmpty() && (System.currentTimeMillis() - startTime < 25000)) {
           ...
           //logger.log(Level.INFO, "fetch complete for: {0}", StringUtils.join(curGeocellsUnique, ", "));
           ...
           // Sort is needed only if new_results.size() > max_results.
           
           if (newResults.size() > maxResults) {
        	   Collections.sort(newResults);
        	   newResults = newResults.subList(0, maxResults);
           }   
           ...
           if(results.isEmpty() || curGeocells.size() == 4) {
           ...
           if(results.size() < maxResults) {
               //logger.log(Level.INFO, "{0} results found but want {1} results, continuing search.", new Object[]{results.size(), maxResults});
               continue;
           }

           //logger.log(Level.INFO, "{0} results found.", results.size());
           ...
           if (closestPossibleNextResultDist >=
               ...
               //logger.log(Level.INFO, "DONE next result at least {0} away, current farthest is {1} dist", new Object[]{closestPossibleNextResultDist, currentFarthestReturnableResultDist});
               break;
           }
           //logger.log(Level.INFO, "next result at least {0} away, current farthest is {1} dist", new Object[]{closestPossibleNextResultDist, currentFarthestReturnableResultDist});
       }
       ...
       for(Tuple<T, Double> entry : results.subList(0, Math.min(maxResults, results.size()))) {
           if(maxDistance == 0 || entry.getSecond() < maxDistance) {
        	   ...
        	   //logger.log(Level.INFO, "Discarding result {0} because distance {1}m > max distance {2}m", new Object[]{entry.getFirst(), entry.getSecond(), maxDistance});
           //}
       }
       logger.log(Level.INFO, "Proximity query looked in {0} geocells and found {1} results.", new Object[]{searchedCells.size(), result.size()});
       return result;
   }
   
   public static final <T> boolean proximityIsEmptySearch(Point center, double maxDistance, Class<T> entityClass, GeocellQuery baseQuery, PersistenceManager pm) {
       ...
       while(!curGeocells.isEmpty()) {
           ...
           results = queryEngine.isEmptyResult(baseQuery, curGeocellsUnique, entityClass);
           if (!results) {
        	   results = false;
        	   break;
           }
       ...
       logger.log(Level.INFO, "proximityIsEmptySearch() " + results);
       return results;
   }
