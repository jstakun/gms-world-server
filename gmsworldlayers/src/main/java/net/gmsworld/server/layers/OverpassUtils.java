...
public class OverpassUtils extends LayerHelper {
	...
	protected List<ExtendedLandmark> loadLandmarks(double lat, double lng, String query, int radius, int version, int limit, int stringLimit, String amenity, String bbox, Locale locale, boolean useCache) throws Exception {
        ...
        if (amenity != null && bbox != null) {
        	...
        	output = createCustomLandmarkList(response, locale);
	...
	private List<ExtendedLandmark> createCustomLandmarkList(String responseJson, Locale locale) throws JSONException {
    	 ...
    	 if (StringUtils.startsWith(responseJson, "{")) {
             ...
             if (elements != null) {
                 for (int i = 0; i < elements.length(); i++) {
                     ...
                     ExtendedLandmark landmark = LandmarkFactory.getLandmark(name, null, qc, amenity, address, creationDate, null);
