...
public class CouponsProviderServlet extends HttpServlet {
	...
	private static final Logger logger = Logger.getLogger(CouponsProviderServlet.class.getName());
    /**
        ...
        try {
            if (HttpUtils.isEmptyAny(request, "latitude", "longitude", "radius")) {
                ...
                if (GeocodeUtils.isNorthAmericaLocation(request.getParameter("latitude"), request.getParameter("longitude"))) {
                    ...
                    int limit = NumberUtils.getInt(request.getParameter("dealLimit"), 300); //max 500
                    ...
                    resp = LayerHelperFactory.getCouponsUtils().processRequest(latitude, longitude, null, radius, version, limit, stringLimit, categoryid, null).toString();
                }
