	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (requestToken != null) {
				...
				// OAuthTokenPersistenceUtils.persistOAuthToken(Commons.TWITTER,
				// accessToken.getToken() + " " + accessToken.getTokenSecret(),
				// username, password, me.getScreenName());
				...
				// move to task
				// TwitterUtils.sendMessage(null, ConfigurationManager.SERVER_URL,
				// accessToken.getToken(), accessToken.getTokenSecret(),
				// Commons.LOGIN);
				// MailUtils.sendUserCreationNotification("User " +
				// ConfigurationManager.SERVER_URL + "socialProfile?uid=" +
				// userData.get(ConfigurationManager.TWEET_USERNAME) + "@" +
				// Commons.TWITTER + " logged in");
				// if (userData.containsKey(ConfigurationManager.USER_EMAIL)) {
				// MailUtils.sendLoginNotification(userData.get(ConfigurationManager.USER_EMAIL),
				// userData.get(ConfigurationManager.TWEET_NAME), "Twitter",
				// getServletContext());
				// }
				//
				...
				queue.add(withUrl("/tasks/notificationTask")
			        ...
			        // param("email",
			        // userData.containsKey(ConfigurationManager.USER_EMAIL) ?
			        // userData.get(ConfigurationManager.USER_EMAIL) : "").
