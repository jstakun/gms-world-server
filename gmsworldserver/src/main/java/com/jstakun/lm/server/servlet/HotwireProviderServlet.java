 ...
 */
@Deprecated
public class HotwireProviderServlet extends HttpServlet {

    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(HotwireProviderServlet.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (HttpUtils.isEmptyAny(request, "latitude", "longitude", "radius")) {
                ...
                int limit = NumberUtils.getInt(request.getParameter("dealLimit"), 50);
                int stringLimit = StringUtil.getStringLengthLimit(request.getParameter("display"));

                String output = LayerHelperFactory.getHotwireUtils().processRequest(latitude, longitude, null, radius, version, limit, stringLimit, null, null).toString();
