 ...
 * @author jstakun
 */
public class FoursquareRecommendedServlet extends HttpServlet {

    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(FoursquareRecommendedServlet.class.getName());
    ...
    throws ServletException, IOException {
        ...
        try {
            if (HttpUtils.isEmptyAny(request,"lat","lng","radius","token") && HttpUtils.isEmptyAny(request,"latitude","longitude","radius","token")) {
                ...
                if (format.equals("bin")) {
                	List<ExtendedLandmark> landmarks = FoursquareUtils.exploreVenuesToLandmark(latitude, longitude, query, radius * 1000, limit, version, token, language, request.getLocale());
                	//outObj.writeObject(landmarks);
            		//outObj.flush();
                	LayerHelperFactory.getFoursquareUtils().serialize(landmarks, response.getOutputStream(), version);
                } else {
                	String json = FoursquareUtils.exploreVenuesToJSon(latitude, longitude, query, radius * 1000, limit, version, token, language);
                	out.print(json);
                }
            }
        } catch (Exception e) {
