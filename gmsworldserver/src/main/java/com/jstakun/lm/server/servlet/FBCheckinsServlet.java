 ...
 * @author jstakun
 */
public class FBCheckinsServlet extends HttpServlet {

    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(FBCheckinsServlet.class.getName());
    ...
    throws ServletException, IOException {
        ...
        try {
            if (HttpUtils.isEmptyAny(request,"lat","lng","token") && HttpUtils.isEmptyAny(request,"latitude","longitude","token")) {
                ...
                if (format.equals("bin")) {
                	List<ExtendedLandmark> landmarks = FacebookUtils.getFriendsCheckinsToLandmarks(latitude, longitude, version, limit, stringLimit, token, request.getLocale());
                	//outObj.writeObject(landmarks);
            		//outObj.flush();
                	LayerHelperFactory.getFacebookUtils().serialize(landmarks, response.getOutputStream(), version);
                } else {
                	String json = FacebookUtils.getFriendsCheckinsToJSon(latitude, longitude, version, limit, stringLimit, token);
